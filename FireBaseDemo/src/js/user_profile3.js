...
/*====================下載json檔===============================*/
function download_journey(element,journey){
    var data = "text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(journey[String(element.id).substring(8)]));
    // what to return in order to show download window?
    //console.log(String(element.id).substring(8));
    //console.log(journey[String(element.id).substring(8)])
    ...
    if (journey_row_count > 0) { //有行程資料
        ...
        for (var i = 0; i < journey_row_count; i++) { //透過迴圈依序把資料放進資料表裡面
            ...
            row = `
                ...
                <td id="journey_download"><a id="journey_`+ journey[Object.keys(journey)[i]]['start_time'] + `">點擊下載</a></td>
            `;
